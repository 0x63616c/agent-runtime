...
An all-interface listener requires explicit `"public_listen":true`. The
reviewed Stack opts in only for its separate `runtime-api` Pod, which has its
own PostgreSQL/content and bearer-token Secret references. Its
`--config-env RUNTIME_API_CONFIG --check` probe validates the strict document
and every referenced API/storage credential without opening a listener or

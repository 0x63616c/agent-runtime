...
func resolveControlOperationRequest(input []byte, acceptedAt, retentionExpiresAt time.Time, policy OperationAdmissionPolicy) (ResolvedOperation, error) {
	...
	return ResolvedOperation{Request: frozen, Operation: operation, ResourceLimits: effective.limits, InputDigest: canonicalDigestBytes(input), CleanupRequired: frozen.Kind != OperationApproveSensitive}, nil
}

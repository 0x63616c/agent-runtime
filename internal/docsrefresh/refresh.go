...
func renderOpenAPIOperationIndex(root string, artifact Artifact, files Files) ([]byte, error) {
	...
	if err := json.Unmarshal(content, &document); err != nil {
		return nil, fmt.Errorf("decode declared OpenAPI contract %q: %w", artifact.Inputs[0], err)
	}
	if document.OpenAPI == "" || document.Info.Title == "" || document.Info.Version == "" || len(document.Paths) == 0 {
		return nil, fmt.Errorf("validate declared OpenAPI contract %q: missing version, info, or paths", artifact.Inputs[0])
	}
...
func (document openAPIDocument) operations() ([]openAPIOperation, error) {
	...
	for path, item := range document.Paths {
		if !strings.HasPrefix(path, "/") || strings.ContainsAny(path, "\r\n|`") {
			return nil, fmt.Errorf("invalid OpenAPI path %q", path)
		}
		for name, rawOperation := range item {
			method, isMethod := methods[name]
			if !isMethod {
				if _, isIgnored := ignored[name]; isIgnored {
					continue
				}
				return nil, fmt.Errorf("unsupported OpenAPI path item field %q", name)
			}
			var decoded struct {
			...
			if !validOpenAPIOperationID(decoded.OperationID) || len(decoded.Responses) == 0 {
				return nil, fmt.Errorf("%s %s is missing a safe operation ID or responses", method, path)
			}
			...
			if len(successes) == 0 {
				return nil, fmt.Errorf("%s %s has no successful response", method, path)
			}
			sort.Strings(successes)
			operations = append(operations, openAPIOperation{ID: decoded.OperationID, Method: method, Path: path, SuccessResponses: successes})
		}
	}
	if len(operations) == 0 {

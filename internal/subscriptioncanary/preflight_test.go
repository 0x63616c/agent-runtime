...
func TestLoadRequiresBoundedOpaqueControlsWithoutRetainingValues(t *testing.T) {
	...
	for _, name := range []string{"AR_SUBSCRIPTION_CANARY_RUNNER_CONTRACT", "AR_SUBSCRIPTION_CANARY_GITHUB_ENVIRONMENT", "AR_SUBSCRIPTION_CANARY_RUNNER_LABELS", "GITHUB_WORKFLOW", "GITHUB_REF_PROTECTED", "RUNNER_ENVIRONMENT", "AR_SUBSCRIPTION_CANARY_CAPABILITY_ENV", "AR_SUBSCRIPTION_CANARY_CREDENTIAL_ENV", "AR_SUBSCRIPTION_CANARY_MODEL_PROFILE", "AR_SUBSCRIPTION_CANARY_REVISION", "AR_SUBSCRIPTION_CANARY_TIMEOUT", "AR_SUBSCRIPTION_CANARY_CANCEL_MODE", "AR_SUBSCRIPTION_CANARY_RECOVERY_MODE"} {
		delete(values, name)
...
func TestLoadRejectsUnsafeOrIncompleteCanaryInputs(t *testing.T) {
	...
	for name, value := range map[string]string{
		"AR_SUBSCRIPTION_CANARY_RUNNER_CONTRACT":    "unsafe",
		"AR_SUBSCRIPTION_CANARY_GITHUB_ENVIRONMENT": "unprotected",
		"AR_SUBSCRIPTION_CANARY_RUNNER_LABELS":      "self-hosted,linux,x64",

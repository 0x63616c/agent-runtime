...
func transferEnvelope(t *testing.T, now time.Time, request sandbox.CopyInRequest) sandboxhostprotocol.Envelope {
	t.Helper()
	payload, err := json.Marshal(GuestTransferCommand{Version: GuestTransferOperationKind, CopyIn: &request})
	if err != nil {
		t.Fatal(err)
	}
...
type snapshotSink struct{ calls int }

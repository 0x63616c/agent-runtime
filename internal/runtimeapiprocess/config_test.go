...
func TestConfigurationIsStrictAndRequiresExplicitUnsafeMemoryStorage(t *testing.T) {
	...
	for _, address := range []string{"0.0.0.0:8088", "[::]:8088"} {
		bound := strings.Replace(validConfig, `"listen_address": "127.0.0.1:8088"`, `"listen_address":"`+address+`","public_listen":true`, 1)
		if _, err := runtimeapiprocess.Parse(strings.NewReader(bound)); err != nil {
			t.Fatalf("Parse(all-interface bind %q) error = %v", address, err)
		}
	}
...
func TestCheckRequiresEveryDeclaredEnvironmentValueWithoutStartingThePublicListener(t *testing.T) {
	...
	if err := runtimeapiprocess.Check(config, mapLookup(map[string]string{"ADMIN_TOKEN": "admin-token-0000", "USER_TOKEN": "user-token-00000"})); err == nil {
		t.Fatal("Check(missing durable environment) error = nil")
	}
	if err := runtimeapiprocess.Check(config, mapLookup(map[string]string{
		...
		t.Fatalf("Check(complete durable environment): %v", err)
	}
}

...
func Parse(input io.Reader) (Config, error) {
	...
	if err != nil || port == "" || (host != "127.0.0.1" && host != "::1" && host != "0.0.0.0") {
		return Config{}, errors.New("validate runtime API configuration: listen_address must be an explicit loopback or Kubernetes bind address")
	}

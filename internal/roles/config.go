...
func validateWorker(role Role, worker *WorkerConfig, localDemo *LocalDemoWorkerConfig) error {
	if localDemo != nil {
		...
		if !localDemo.Enabled {
			if worker != nil || localDemo.Mode != "disabled" || localDemo.StateDSNEnvironment != "" || localDemo.ContentEndpoint != "" || localDemo.ContentAccessKeyEnvironment != "" || localDemo.ContentSecretKeyEnvironment != "" || localDemo.ContentBucket != "" {
				return errors.New("validate runtime role configuration: disabled local demo worker capability is invalid")
			}
			return nil
		}
		if worker != nil || localDemo.Mode != "local-demo-v1" || localDemo.StateDSNEnvironment != "LOCAL_DEMO_STATE_DSN" || !validEndpoint(localDemo.ContentEndpoint) || localDemo.ContentAccessKeyEnvironment != "LOCAL_DEMO_CONTENT_ACCESS_KEY" || localDemo.ContentSecretKeyEnvironment != "LOCAL_DEMO_CONTENT_SECRET_KEY" || !validWorkerSegment(localDemo.ContentBucket) {

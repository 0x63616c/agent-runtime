...
var _ = Describe("Stack admission policy", func() {
	It("admits explicit bounded resources and rejects unsafe infrastructure defaults", func() {
		...
		invalid := []string{
			...
			resourceStack(kubernetesResource("dns-without-subject", `{"api_version":"networking.k8s.io/v1","kind":"NetworkPolicy","name":"dns-without-subject","network":{"default_deny":true,"allow_dns":true,"allowed_egress":[]}}`)),

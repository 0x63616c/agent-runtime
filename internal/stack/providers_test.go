...
var _ = Describe("Declared provider reconciliation", func() {
	It("reconciles the exact blob prefix and verifies database, telemetry, and secret declarations", func() {
		...
		runner := &fakeProviderRunner{expectedDigest: rendered.Digest()}
		...
		Expect(joinedProviderCommands(runner.commands)).To(ContainSubstring("get EndpointSlice --namespace ar-feature-a --selector kubernetes.io/service-name=telemetry"))
		Expect(joinedProviderCommands(runner.commands)).To(ContainSubstring("psql -At -U runtime-owner -d runtime"))
	...
	It("rejects undeclared secret keys instead of widening workload authority", func() {
		rendered := renderProviderStack("delete")
		runner := &fakeProviderRunner{extraKey: true, expectedDigest: rendered.Digest()}
		adapter, err := stack.NewKubectlDeclaredProviderAdapter(runner)
		Expect(err).NotTo(HaveOccurred())
	...
	It("rejects a local-generated secret not bound to the reviewed render", func() {
		rendered := renderProviderStack("delete")
		runner := &fakeProviderRunner{expectedDigest: "wrong-render-digest"}
		adapter, err := stack.NewKubectlDeclaredProviderAdapter(runner)
		Expect(err).NotTo(HaveOccurred())
...
{"id":"blob-creds","kind":"secret_reference","owner":"security-operator","scope":"namespace","dependencies":[],"retention":%[1]s,"backup_restore_owner":"none","delete_behavior":"%[2]s","external_controller":true,"secret_reference":{"provider":"local-generated","reference":"blob-creds","version":"v1","keys":["MINIO_ROOT_PASSWORD","MINIO_ROOT_USER"]}},
{"id":"db-creds","kind":"secret_reference","owner":"security-operator","scope":"namespace","dependencies":[],"retention":%[1]s,"backup_restore_owner":"none","delete_behavior":"%[2]s","external_controller":true,"secret_reference":{"provider":"local-generated","reference":"db-creds","version":"v1","keys":["POSTGRES_PASSWORD"]}},
...
{"id":"telemetry","kind":"kubernetes","owner":"telemetry-operator","scope":"namespace","dependencies":["runtime-account"],"retention":%[1]s,"backup_restore_owner":"none","delete_behavior":"%[2]s","external_controller":false,"kubernetes":{"api_version":"apps/v1","kind":"Deployment","name":"telemetry","image":"registry.invalid/telemetry@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd","service_account":"runtime-account","environment":[{"name":"BADGER_SPAN_STORE_TTL","value":"24h"}],"ports":[{"name":"otlp","number":4318,"protocol":"TCP"}],"compute":{"request_milli_cpu":100,"limit_milli_cpu":500,"request_memory_bytes":134217728,"limit_memory_bytes":268435456},"storage":[]}},

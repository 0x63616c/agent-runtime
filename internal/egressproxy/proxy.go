...
func publicAddress(address net.IP) bool {
	if address == nil || address.IsLoopback() || address.IsLinkLocalUnicast() || address.IsLinkLocalMulticast() || address.IsMulticast() || address.IsUnspecified() || address.IsPrivate() {
		return false
	}
	if ipv4 := address.To4(); ipv4 != nil {
		return ipv4[0] != 0 && ipv4[0] < 224
	}
	return true
}

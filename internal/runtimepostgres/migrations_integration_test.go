...
func TestRuntimeV5PartitionsAndRLSRejectUnboundOrCrossTenantStateAccess(t *testing.T) {
...
func TestRuntimeMigrationResetRemovesV5RetentionSchedulingAuthorityBeforeReapplication(t *testing.T) {
	...
	resetRuntimeV2(t, ctx, pool)
	var retained bool
	if err := pool.QueryRow(ctx, `SELECT to_regclass('runtime.tenant_retention_jobs') IS NOT NULL`).Scan(&retained); err != nil {
		t.Fatalf("read retained v5 retention scheduling authority: %v", err)
	}
	if retained {
		t.Fatal("reset retained v5 tenant retention scheduling authority")
	}
	...
	if err := pool.QueryRow(ctx, `
		...
		WHERE pg_namespace.nspname = 'runtime'
		  AND parent.relname = 'tenant_retention_jobs'`).Scan(&partitions); err != nil {
		t.Fatalf("count re-applied retention job partitions: %v", err)
	}
	if partitions != 4 {
		t.Fatalf("re-applied retention job partitions = %d, want 4", partitions)
	}
}
...
func applyRuntimeMigrations(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	for _, migration := range []struct {
		version  int
		filename string
	}{{1, "runtime-v1.up.sql"}, {2, "runtime-v2.up.sql"}, {3, "runtime-v3.up.sql"}, {4, "runtime-v4.up.sql"}, {5, "runtime-v5.up.sql"}} {

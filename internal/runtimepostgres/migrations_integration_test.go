...
func TestRuntimeV5PartitionsAndRLSRejectUnboundOrCrossTenantStateAccess(t *testing.T) {
	...
	var all int
	if err := operator.QueryRow(ctx, `SELECT count(*) FROM runtime.runtime_state_snapshots`).Scan(&all); err != nil {
		t.Fatalf("query operator state: %v", err)
	}
	if all != 2 {
		t.Fatalf("operator state rows = %d, want 2", all)
	}
	var partitions int

-- Named volumes are a first-class resource projection. The generic JSON
-- projection table remains principal-scoped; this only widens the closed
-- operation/resource binding to the reviewed volume kind.
ALTER TABLE runtime.sandbox_resource_projections
    ...
    ADD CONSTRAINT sandbox_resource_projections_resource_kind_check
        CHECK (resource_kind IN ('sandbox', 'process', 'volume'));
...
ALTER TABLE runtime.sandbox_operations
    ...
    ADD CONSTRAINT sandbox_operations_resource_projection_binding_check CHECK (
        ...
        OR
        (resource_projection_kind IN ('sandbox', 'process', 'volume')

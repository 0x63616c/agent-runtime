...
def local_demo_fixture($namespace; $profile):
  if $profile != "local" then .
    ...
    .kubernetes.environment |= map(
      if .name == "RUNTIME_ROLE_CONFIG" then
        .value |= (fromjson |
          .local_demo_worker = {enabled:true,mode:"local-demo-v1",fixture:"workspace-approval-v1",state_dsn_environment:"LOCAL_DEMO_STATE_DSN",content_endpoint:("blob." + $namespace + ".svc:9000"),content_access_key_environment:"LOCAL_DEMO_CONTENT_ACCESS_KEY",content_secret_key_environment:"LOCAL_DEMO_CONTENT_SECRET_KEY",content_bucket:$namespace} |
          tojson)
      else . end) |

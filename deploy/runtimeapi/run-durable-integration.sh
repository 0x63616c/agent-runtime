...
go test -race -tags=integration ./internal/runtimeapi ./internal/runtimeapiprocess ./internal/runtimemodel ./internal/runtimeorchestration ./internal/runtimetool -run 'Test(DurableStateRuntimeAuthorizesArtifactInputReferences|DurablePostgresMinIOCollectorDeletesExpiredUnreferencedArtifact|DurablePostgresMinIOTenantErasurePreservesAnotherTenant|DurablePostgresMinIOAPIProcessSurvivesRestart|DurableWorkspaceAgentBinariesUseOnlyThePublicAPI|ResearchDossierRecoversLongRunningToolResearchThroughThePublicContract|DurableModelProducerLossFinalizesGapAndReplays|DurableModelNormalizedStreamFinalizesOwnerReadableOutput|CodecEnabledWorkerStartsAgainstDurableDependenciesAndRestarts|PostgresOutboxReclaimsLiveTemporalRouteAfterAcknowledgementLoss|PostgresOutboxRecoversAcrossActualPublisherProcessKills|PostgresOutboxReclaimsAuditExportAfterSinkOutage|SessionWorkflowReplaysCheckedInHistoricLifecycle|DurableToolLifecyclePersistsDescriptorApprovalAndFinalization)' -count=1

# Keep the shipped M8 application proof as a separate invocation. Besides
# making its PostgreSQL/MinIO/Temporal result unmistakable in gate output,
# this prevents a broad package selector from accidentally skipping the
# application-level durable-restart scenario.

...
SDK and HTTP adapter are checked against its fifteen operations by `just check`.
...
events after an opaque Cursor, explicitly cancel a Turn, cancel a drained
Session, and drain-close a Session, download caller-authorized immutable Artifacts, and inspect or decide
...
## Optional request-completion telemetry

The standalone role emits no public `/v1` API completion telemetry by default.
To opt into one JSON `slog` record plus OpenTelemetry-compatible completion
span and metrics for every public API completion, add this strict
...
The key is an explicit secret of 32 to 4096 bytes. Each completion has a
canonical operation name, request ID, HTTP status, safe outcome/failure code,
duration, and keyed HMAC tenant/principal correlations. The JSON record and
trace contain only those safe values. Metrics intentionally use only the
bounded operation, status, outcome, and failure-code dimensions; they never
carry request IDs or tenant/principal correlations. The role sends its spans
and metrics to the active process-global OpenTelemetry provider, so the
operator must install and configure that provider at the composition root.
It never contains the key, bearer token, idempotency key, request body, raw
identity, raw URL, or backend identifier. This is not yet an OTel exporter,
dashboard, durable audit record, or cross-service correlation proof.

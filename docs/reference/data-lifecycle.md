...
| Outbox records | `outbox` | Durable publication/reconciliation lease metadata. | Independent outbox retention. A route delivered before its acknowledgement is lease-reclaimed and repeated at least once; the workflow deduplicates the durable sequence. A live tool worker renews only the exact current lease version it owns through descriptor read, external execution, terminal persistence, and acknowledgement; an expired or superseded claim is reconciled under the same operation ID rather than re-executed. |

...
| Audit facts | `audit` | Tenant-scoped append-only redacted facts. | Independent audit retention; it is not shortened because a related event or approval expires. An explicitly composed HTTP audit exporter receives the exact concurrently committed fact through its own durable outbox route. Sink failure leaves that route lease-reclaimable and delivery at-least-once; no transactional fail-closed behavior is claimed. The default worker has no mandatory external audit-sink configuration. |

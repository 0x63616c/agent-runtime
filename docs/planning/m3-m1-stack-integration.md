...
| Host lifecycle | `sandbox-host` performs one bounded poll and exits with `ErrNoWork`. | Add a cancellable daemon/run-loop and bounded readiness that treats verified no-work as healthy; do not deploy the current command as a Deployment. |

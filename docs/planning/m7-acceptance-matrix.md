...
| TOL-006 | S5 Tool broker | MCP cancellation and status-only reconciliation return uncertainty rather than re-submit an effect; the disposable PostgreSQL/MinIO proof reclaims a lost worker lease after reopening the runtime store, reconciles the exact persisted operation ID, retains an uncertain public terminal state with no Artifact, and records no second external submission. | SHA-bound retained evidence entry for the durable uncertain-outcome/recovery proof. | unclaimed |

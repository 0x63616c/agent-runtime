...
Status: implementation evidence only. This document does not promote a ledger
requirement, claim a production deployment, or change the acceptance status of
any later runtime role.
...
| PAY-003 | The public bounded `BlobStore`, content-addressed key format, local converter, authenticated UI handler, metrics observer, and MinIO/S3 adapter have conformance tests. |
...
| PAY-008 | Frozen inline/zstd/remote vectors, two independent local consumers, and authenticated UI inspection prove representation exchange through the public codec seam. |
| TMP-005/TMP-006 | The one runtime factory installs the local converter and performs a startup compatibility round trip before returning usable composition. |

...
| --- | --- | --- |
| DOM-001–011 (bounded foundation) | Immutable Agent revisions, revision-pinned Sessions, bounded Input references, serialized Turns, invocation records/fences, cancellation and settlement are implemented by `internal/runtimestate`; `compiler_planner_test.go`, `state_runtime_test.go`, and durable API integration cover the initial lifecycle. `AppendConversation` persists only immutable content references under expected-version conflict/idempotency, while `RegisterArtifact` and the state-authorized content reader provide ID/digest metadata plus principal-checked readback. An approved Approval creates a bounded capability grant, and worker consumption enforces owner, policy revision, expiry, and maximum uses before an external effect. `internal/runtimetool` reads only the state-authorized immutable sandbox-control descriptor, refuses missing/corrupt/cross-scope descriptors before execution, and reconciles a lost claim by the exact operation ID without resubmission. Its terminal observation persists a correlated `sandbox_operation.finalized` product event/outbox route, which the Temporal publisher and durable dispatcher recheck. `internal/runtimemodel` now owns a provider-neutral invocation adapter: fresh intents invoke once, expired claims reconcile by the same operation ID, and both persist a fenced terminal result plus optional unknown-preserving token usage before acknowledgement. | Artifact input authorization is not wired through `StateRuntime`; conversation/model production-provider composition and normalized streaming remain absent. The tool path still needs full public API and non-fake sandbox-control process evidence. |

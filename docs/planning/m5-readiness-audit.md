...
| TMP-002–004 (Session route) | Private Temporal-free public API boundary; owned codec factory; durable state/outbox scheduler; replay-safe Session workflow versioning, duplicate tolerance, Continue-As-New, and development-server replay evidence. The owned Temporal development-server test now records and replays an actual Input → Approval → sandbox-finalization → Session-complete history through the registered private workflow. The disposable PostgreSQL/MinIO tool integration persists the exact staged descriptor, approval/grant, execution result, and correlated finalization Event, then reads the durable replay projection back. | A checked-in retained historic replay fixture and one fully composed PostgreSQL-to-Temporal lifecycle remain absent. |
...
- TMP-010: full durable-state and retained-history evidence for the approval,
  ...
  test now covers approval plus uncertain producer-loss finalization, while the
  tool worker now emits and publisher/dispatcher-rechecks a correlated sandbox
  finalization route. Its concrete adapter is also exercised through the HTTPS
  sandbox control handler, but the test control ledger is intentionally
  in-memory. The Temporal test environment still needs a composed PostgreSQL
  durable-state lifecycle and retained-history completion proof.

...
The private Firecracker boot probe is not a v1 assignment consumer. Its distinct
`sandbox.host-control/v2/firecracker-boot-probe` session is recorded in
PostgreSQL under a host-instance session ID and a monotonic CAS version. M3
checks the active mTLS enrollment and the exact current assignment, lease and
fence on creation and every lifecycle transition. Revocation, quarantine,
assignment replacement, stale renewals and stale snapshots are refused; M4
...
multi-process tests prove public Submit, durable body/routing, mTLS host pull,

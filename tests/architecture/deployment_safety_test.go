...
var _ = Describe("M1 deployment safety boundaries", func() {
	...
	It("invokes the real two-Stack isolation proof in a disposable CI cluster", func() {
		...
		for _, required := range []string{
			...
			`diagnostic-summary/v4`,
			`--output-snapshot-on-exit "$snapshot"`,
			`tilt_snapshot_diagnostics`,
			`api_last_termination`,
			`tilt_resource_status`,
			`tilt_snapshot_state`,
			`runtime_role_status`,
			...
			`secret_references_scoped:true`,
			`trust wiring self-test accepted envFrom`,
			`operator namespace describe --namespace "$namespace"`,
			`$namespace-session-v1`,
			`state_service_account:"state-account"`,
			`blob_service_account:"blob-account"`,
			`telemetry_service_account:"telemetry-account"`,
